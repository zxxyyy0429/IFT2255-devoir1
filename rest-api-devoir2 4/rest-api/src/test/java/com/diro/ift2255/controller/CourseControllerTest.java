...
public class CourseControllerTest {
    ...
    @Mock // ← Crée un FAUX Course Service
    ...
    @Test
    @DisplayName("Get all courses should return empty list when no courses exist")
    void testGetAllCoursesEmptyResult() {
         when(mockContext.queryParamMap()).thenReturn(new HashMap<>());
         when(mockService.getAllCourses(any())).thenReturn(Collections.emptyList());

         controller.getAllCourses(mockContext);

         verify(mockService).getAllCourses(any());
         verify(mockContext).json(Collections.emptyList());
    }

    @Test
    @DisplayName("Get all courses should ignore unknown query parameters")
    void testGetAllCoursesWithUnknownParameter() {
         Map<String, List<String>> params = new HashMap<>();
         params.put("unknown", List.of("test"));

         when(mockContext.queryParamMap()).thenReturn(params);
         when(mockService.getAllCourses(any())).thenReturn(Collections.emptyList());
    ...
    void testGetCourseByIdWhenIdIsEmpty() {
     ...
     @Test
     @DisplayName("Get course by ID should return 404 when ID contains spaces")
     void testGetCourseByIdWithSpaces() {
       ...
       // ARRANGE
        ...
        when(mockService.getCourseById(courseId))
       ...
       // ASSERT
        verify(mockService, times(1)).getCourseById(courseId);
        verify(mockContext).status(404);
    ...
    @Test
    @DisplayName("Comparaison impossible si id1 est manquant")
    void testCompareCourses_MissingId1() {
        Mockito.when(mockContext.queryParam("id1")).thenReturn(null);
        Mockito.when(mockContext.queryParam("id2")).thenReturn("ABC123");
        Mockito.when(mockContext.status(400)).thenReturn(mockContext);

        controller.compareCourses(mockContext);

        Mockito.verify(mockContext).status(400);
        Mockito.verify(mockContext).json(Mockito.argThat(map ->((Map<?, ?>) map).containsKey("error")));
    } 


    @Test
    @DisplayName("HRX-5 — Comparaison impossible si id2 est manquant")
    void testCompareCourses_MissingId2() {
        Mockito.when(mockContext.queryParam("id1")).thenReturn("ABC123");
        Mockito.when(mockContext.queryParam("id2")).thenReturn(null);
        Mockito.when(mockContext.status(400)).thenReturn(mockContext);

        controller.compareCourses(mockContext);

        Mockito.verify(mockContext).status(400);
        Mockito.verify(mockContext).json(Mockito.argThat(map ->
            ((Map<?, ?>) map).containsKey("error")));
}

    @Test
    @DisplayName("Comparaison impossible si id1 et id2 sont identiques")
    void testCompareCourses_SameIds() {
       // ARRANGE
        Mockito.when(mockContext.queryParam("id1")).thenReturn("IFT2255");
        Mockito.when(mockContext.queryParam("id2")).thenReturn("IFT2255");
        Mockito.when(mockContext.status(404)).thenReturn(mockContext);

       // ACT
        controller.compareCourses(mockContext);

       // ASSERT
        Mockito.verify(mockContext).status(404);
        Mockito.verify(mockContext).json(Mockito.any());
        Mockito.verify(mockService, Mockito.never())
           .getCourseDetails(Mockito.any(), Mockito.anyMap());
    }


     /**************************************************************************
     * Tests for getCourseDetails method
     *************************************************************************/
    @Test
   ...
   @Test
   @DisplayName("Get course details should return 404 when course does not exist")
   void testGetCourseDetailsNotFound() {
      // ARRANGE
        String courseId = "IFT9999";

        Mockito.when(mockContext.pathParam("id")).thenReturn(courseId);
        Mockito.when(mockContext.status(404)).thenReturn(mockContext);

        Mockito.when(
          mockService.getCourseDetails(Mockito.eq(courseId), Mockito.anyMap())
       ).thenReturn(Optional.empty());

       // ACT
        controller.getCourseDetails(mockContext);

      // ASSERT
        Mockito.verify(mockContext).status(404);
        Mockito.verify(mockContext).json(Mockito.any());
        Mockito.verify(mockService)
           .getCourseDetails(Mockito.eq(courseId), Mockito.anyMap());
    }


    @Test
    @DisplayName("Get course details should return CourseDetails with no available terms")
    void testGetCourseDetailsNoAvailableTerms() {
    // ARRANGE
    String courseId = "IFT0000";

       CourseDetails.AvailableTerms terms = new CourseDetails.AvailableTerms();
       terms.setAutumn(false);
       terms.setWinter(false);
       terms.setSummer(false);

       CourseDetails mockDetails = new CourseDetails();
       mockDetails.setId(courseId);
       mockDetails.setName("Cours spécial");
       mockDetails.setCredits(3);
       mockDetails.setAvailableTerms(terms);
       mockDetails.setPrerequisiteCourses(List.of("IFT1015"));

       Mockito.when(mockContext.pathParam("id")).thenReturn(courseId);

       Mockito.when(
        mockService.getCourseDetails(
            Mockito.eq(courseId),
            Mockito.anyMap()
         )
      ).thenReturn(Optional.of(mockDetails));

    // ACT
     controller.getCourseDetails(mockContext);

    // ASSERT
     Mockito.verify(mockContext).json(Mockito.argThat(obj -> {
        CourseDetails cd = (CourseDetails) obj;
        return cd.getId().equals(courseId)
            && !cd.getAvailableTerms().isAutumn()
            && !cd.getAvailableTerms().isWinter()
            && !cd.getAvailableTerms().isSummer();
        }));
    }
    ...
    /**************************************************************************
     * Tests getCourseByMotCle method
     *************************************************************************/
